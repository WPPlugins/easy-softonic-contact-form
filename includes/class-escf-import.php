...
/**
 * Description of class-escf-import
 * Import class to import settings from pre 1.0 versions of the plugin
...
//print_r(self::$global_options)."<br>\n";
	...
	static function convert_form_options( $old_options, $max_fields ) {
		...
		$new_options = ESCF_Util::get_form_defaults();
		...
		for ($fld=1; $fld<= $max_fields; $fld++){
			...
			if ( ! empty($old_options['ex_field'.$fld.'_label']) || 'fieldset' == $old_type || 'fieldset-close' == $old_type ) {
				...
				$new_field = ESCF_Util::get_field_defaults();
				foreach ( $new_field as $key => $val ) {
					...
					switch ( $key ) {
						...
						case "label":
                            if ( empty($old_options['ex_field'.$fld.'_label']) && ( 'fieldset' == $old_type || 'fieldset-close' == $old_type ) )
                               $old_options['ex_field'.$fld.'_label'] = sprintf( __( 'Field %s', 'easy-softonic-contact-form' ), $fld );
				...
				//$new_field['slug'] = substr( urldecode(sanitize_title_with_dashes(remove_accents($new_field['label']))), 0, ESCF_MAX_SLUG_LEN );
...
//echo 'slug before:'.$new_field['label']."<br>\n";
                ...
                $new_field['slug'] = substr( urldecode(sanitize_title_with_dashes($new_field['slug'])), 0, ESCF_MAX_SLUG_LEN );
                if ($new_field['slug'] == '')
                   $new_field['slug'] = 'na';
				...
				$new_field['slug'] = ESCF_Options::check_slug($new_field['slug'], $slug_list);

...
/**
 * Description of class-escf-process
...
class ESCF_Process {
	...
	static function process_form() {
		...
		if ( isset( $_POST['es_contact_action'] ) && ( 'send' == $_POST['es_contact_action'] )
		...
		if ( self::$form_processed )
         ...
         $token = 'ok';
         if (!isset($_POST['es_postonce_'.self::$form_id_num]) || empty($_POST['es_postonce_'.self::$form_id_num]) || strpos($_POST['es_postonce_'.self::$form_id_num] , ',') === false ) {
           $token = 'bad';
         }
         $vars = explode(',', $_POST['es_postonce_'.self::$form_id_num]);
         ...
         if ( $token == 'bad' ) {
             ...
             self::$form_action_url = ESCF_Display::get_form_action_url();
             wp_redirect( self::$form_action_url );
		     exit;
         }

		self::$global_options = ESCF_Util::get_global_options();
		self::$form_options = ESCF_Util::get_form_options(self::$form_id_num, $use_defauilts=true);
...
/*		global $fsc_enable_ip_bans, $fsc_banned_ips;
		...
		if ( $fsc_enable_ip_bans && in_array( $_SERVER['REMOTE_ADDR'], $fsc_banned_ips ) )
			wp_die( __( 'Your IP is Banned', 'easy-softonic-contact-form' ) );*/
	...
	static function validate_data() {
		...
		foreach ( $special_slugs as $special ) {
			if ( isset($_POST[$special]) ) {
				...
				self::$form_data[$special] = ESCF_Util::clean_input( $_POST[$special] );
			}
		}
		...
		if ( empty( $cid ) ) {
			self::$form_errors['contact'] = ( self::$form_options['error_contact_select'] != '') ? self::$form_options['error_contact_select'] : __( 'Selecting a contact is required.', 'easy-softonic-contact-form' );
		} else  {
			$frm_id = self::$form_id_num;
			$contacts = ESCF_Display::get_contact_list(self::$form_id_num, self::$form_options['email_to']);
			$contact = ( isset($contacts[$cid])) ? $contacts[$cid] : false ;
			if ( ! isset( $contact['CONTACT'] ) ) {
			   self::$form_errors['contact'] =  __( 'Requested Contact not found.', 'easy-softonic-contact-form' );
			}
		}
		// Setup the email and contact name for email
		self::$email_fields['email_to'] = ( isset( $contact['EMAIL'] ) ) ? ESCF_Util::clean_input( $contact['EMAIL'] ) : '';
		self::$email_fields['name_to'] = ( isset( $contact['CONTACT'] ) ) ? ESCF_Util::clean_input( $contact['CONTACT'] ) : '';
        ...
        self::$email_fields['name_to'] = str_replace('&quot;','"',self::$email_fields['name_to']);
        self::$email_fields['name_to'] = str_replace('&amp;','&',self::$email_fields['name_to']);
		self::$email_msg = self::make_bold( __( 'To:', 'easy-softonic-contact-form' ) ) . $inline_or_newline . self::$email_fields['name_to'] .self::$php_eol.self::$php_eol;
		...
		foreach ( self::$form_options['fields'] as $key => $field ) {
			...
			if ( isset( $_POST[$field['slug']] ) ) {
				if ( 'textarea' == $field['type'] && 'true' == self::$form_options['textarea_html_allow'] )
					...
					self::$form_data[$field['slug']] = ESCF_Util::clean_input( $_POST[$field['slug']] );
			}
			...
			if ( in_array($field['type'], self::$select_type_fields) ) {
				//if ( 'checkbox' != $field['type'] ) {
                    ...
                    if ( 'subject' == $field['slug'] && 'select' == $field['type']) {
					   ...
					   self::validate_select( $field['slug'], $field );
						...
						if ( ! isset($_POST[$field['slug']]) ) {
						self::$form_errors[$field['slug']] = (self::$form_options['error_select'] != '') ? self::$form_options['error_select'] : __('At least one item in this field is required.', 'easy-softonic-contact-form');
						}
					}
				//}
				...
				if ( 'full_name' == $field['slug'] )
					...
					self::$form_errors[$field['slug']] = ( self::$form_options['error_field'] != '') ? self::$form_options['error_field'] : __('This field is required.', 'easy-softonic-contact-form');
				}
			}
			...
			if ( in_array($field['type'], self::$text_type_fields) && $field['max_len'] != ''
				...
				self::$form_errors[$field['slug']] = sprintf( ( self::$form_options['error_maxlen'] != '') ? self::$form_options['error_maxlen'] : __('Maximum of %d characters exceeded.', 'easy-softonic-contact-form'), $field['max_len']);
			}
			...
			if ( ! in_array($field['type'], self::$select_type_fields) && 'hidden' != $field['type'] && 'checkbox' != $field['type'] && $field['regex'] != '' ) {
               if ( 'true' == $field['req'] && empty($_POST[$field['slug']]) ) {
                  ...
                  self::$form_errors[$field['slug']] = ($field['regex_error'] != '') ? $field['regex_error'] : __('Invalid input.', 'easy-softonic-contact-form');
               }
			}

           // filter hook for form input validation
           self::$form_errors = apply_filters('es_contact_form_validate', self::$form_errors, self::$form_id_num);
			...
			switch ( $field['type'] ) {
					...
					if ( 'full_name' != $field['slug'] && 'email' != $field['slug']  ) {
                        if ( self::$form_data[$field['slug']] == '' && self::$form_options['email_hide_empty'] == 'true' ) {
                                ...
                                if ( 'subject' == $field['slug'] ) {
                                        $this_label = (self::$form_options['title_subj'] != '') ? self::$form_options['title_subj'] : __( 'Subject:', 'easy-softonic-contact-form' );
                                        self::$email_msg .= self::make_bold( $this_label ) . $inline_or_newline;
                                } elseif ( 'message' == $field['slug'] ) {
                                        $this_label = (self::$form_options['title_mess'] != '') ? self::$form_options['title_mess'] : __( 'Message:', 'easy-softonic-contact-form' );
                                        self::$email_msg .= self::make_bold( $this_label ) . $inline_or_newline;
                                } else {
                                        self::$email_msg .= self::make_bold( $field['label'] ) . $inline_or_newline;
                                }
				...
				case 'checkbox' :
                    if ( empty( self::$form_data[$field['slug']] ) && self::$form_options['email_hide_empty'] == 'true' ) {
					        ...
					        if ( '1' == self::$form_data[$field['slug']] ) {
                               self::$email_msg .= self::make_bold( $field['label'] ) . $inline_or_newline;
					           //self::$email_fields[$field['slug']] = '* '.__('selected', 'easy-softonic-contact-form');
                               self::$email_fields[$field['slug']] = __('selected', 'easy-softonic-contact-form');
				...
				case 'date' :
					...
					if( 'true' != $field['req'] && ( $cal_date_array[self::$form_options['date_format']] == $_POST[$field['slug']] || empty($_POST[$field['slug']]) ) ) { // not required, no date picked
						...
						self::$form_errors[$field['slug']] = sprintf( (self::$form_options['error_date'] != '') ? self::$form_options['error_date'] : __('Please select a valid date in this format: %s.', 'easy-softonic-contact-form'), $cal_date_array[self::$form_options['date_format']] );
					} else {
				...
				case 'time' :
				   ...
				   if ( self::$form_options['time_format'] == '12' ) {
                        ...
                        if( 'true' != $field['req'] && ( empty(self::$form_data[$field['slug']]['h']) && empty(self::$form_data[$field['slug']]['m']) && empty(self::$form_data[$field['slug']]['ap']) ) ) { // not required, no time picked
					         ...
					         self::$form_errors[$field['slug']] = ( self::$form_options['error_time'] != '') ? self::$form_options['error_time'] : __('The time selections are incomplete, select all or none.', 'easy-softonic-contact-form');
			...
			foreach ( $hidden_fields as $key => $value ) {
				if ( $key != '' && $value != '' ) {
					if ( $key == 'form_page' ) {  // page url
						self::$email_msg .= self::make_bold( __( 'Form Page', 'easy-softonic-contact-form' ) ) . $inline_or_newline. esc_url( self::$form_action_url ) . self::$php_eol . self::$php_eol;
        ...
        self::$email_fields = apply_filters('es_contact_email_fields', self::$email_fields, self::$form_id_num);

        // filter hook to add any custom fields to email message (not validated)
        self::$email_msg = apply_filters('es_contact_email_msg', self::$email_msg, $inline_or_newline, self::$php_eol, self::$form_id_num);
        ...
        if ( self::$form_options['honeypot_enable'] == 'true' && !isset(self::$form_errors['captcha'])) {
           $honeypot_slug = ESCF_Display::get_todays_honeypot_slug($fields_in_use);
           if ( !empty($_POST[$honeypot_slug] ) )
           self::$form_errors[$honeypot_slug] = (self::$form_options['error_spambot'] != '') ? self::$form_options['error_spambot'] : __('Possible spam bot. Try again.', 'easy-softonic-contact-form');
        }
        ...
        self::$email_msg = apply_filters('es_contact_email_message', self::$email_msg, self::$email_fields, $inline_or_newline, self::$php_eol, self::$form_id_num);
    ...
    if ( ! self::$form_options )
		   self::$form_options = ESCF_Util::get_form_options ( $form_id_num, $use_defaults=true );
	...
	static function validate_name($field, $inline_or_newline) {
			...
			switch ( self::$form_options['name_format'] ) {
				case 'name':
					if ( '' == self::$form_data['full_name'] || $placeh_name_fail) {
						self::$form_errors['full_name'] = (self::$form_options['error_name'] != '') ? self::$form_options['error_name'] : __( 'Your name is required.', 'easy-softonic-contact-form' );
                        if ($placeh_name_fail)
				...
				default:
                  // middle initial is allowed to be empty
					if ( empty( self::$form_data['f_name'] ) || $placeh_fname_fail ) {
						self::$form_errors['f_name'] = (self::$form_options['error_name'] != '') ? self::$form_options['error_name'] : __( 'Your name is required.', 'easy-softonic-contact-form' );
                        if ($placeh_fname_fail)
                           self::$form_data['f_name'] = $f_default;
                    }
					if ( empty( self::$form_data['l_name'] ) || $placeh_lname_fail ) {
						self::$form_errors['l_name'] = (self::$form_options['error_name'] != '') ? self::$form_options['error_name'] : __( 'Your name is required.', 'easy-softonic-contact-form' );
		...
		foreach ( array('full_name','f_name','m_name','l_name') as $fld) {
			if ( ! empty(self::$form_data[$fld]))
				self::$form_data[$fld] = ESCF_Util::name_case(self::$form_data[$fld]);
		}
		...
		switch ( self::$form_options['name_format'] ) {
			case 'name':
                if ( self::$form_data['full_name'] == '' && self::$form_options['email_hide_empty'] == 'true' ) {
                        ...
                        $this_label = (self::$form_options['title_name'] != '') ? self::$form_options['title_name'] : __( 'Name:', 'easy-softonic-contact-form' );
                        self::$email_msg .= self::make_bold( $this_label ) . $inline_or_newline;
					    self::$email_msg .= self::$form_data['full_name'] . self::$php_eol . self::$php_eol;
                }
				break;
			case 'first_last':
				self::$email_msg .= (self::$form_options['title_fname'] != '') ? self::$form_options['title_fname'] : __( 'First Name:', 'easy-softonic-contact-form' );
				self::$email_msg .= ' ' . self::$form_data['f_name'] . self::$php_eol;
				self::$email_msg .= (self::$form_options['title_lname'] != '') ? self::$form_options['title_lname'] : __( 'Last Name:', 'easy-softonic-contact-form' );
			...
			case 'first_middle_i_last':
				...
				if ( self::$form_data['mi_name'] != '' && !$placeh_miname_fail ) {
					self::$email_msg .= (self::$form_options['title_miname'] != '') ? self::$form_options['title_miname'] : __( 'Middle Initial:', 'easy-softonic-contact-form' );
			...
			case 'first_middle_last':
				...
				if ( self::$form_data['m_name'] != '' && !$placeh_mname_fail ) {
					self::$email_msg .= (self::$form_options['title_mname'] != '') ? self::$form_options['title_mname'] : __( 'Middle Name:', 'easy-softonic-contact-form' );
	...
	static function validate_email($req, $inline_or_newline) {
		...
		if ( isset( $_POST['email'] ) )
			$email = strtolower( ESCF_Util::clean_input( $_POST['email'] ) );
		if ( 'true' == self::$form_options['double_email'] ) {
            $req = 'true';
			if ( isset( $_POST['email2'] ) )
				$email2 = strtolower( ESCF_Util::clean_input( $_POST['email2'] ) );
		}

		if ( 'true' == $req ) {
			if ( ! ESCF_Util::validate_email( self::$form_data['email'] ) ) {
				self::$form_errors['email'] = (self::$form_options['error_email'] != '') ? self::$form_options['error_email'] : __( 'A proper email address is required.', 'easy-softonic-contact-form' );
			}
			if ( 'true' == self::$form_options['double_email'] && ! ESCF_Util::validate_email( $email2 ) ) {
				self::$form_errors['email2'] = (self::$form_options['error_email'] != '') ? self::$form_options['error_email'] : __( 'A proper email address is required.', 'easy-softonic-contact-form' );
			}
			if ( 'true' == self::$form_options['double_email'] && !empty($email) && !empty($email2) && ($email != $email2) ) {
				self::$form_errors['email2'] = (self::$form_options['error_email2'] != '') ? self::$form_options['error_email2'] : __( 'The two email addresses did not match.', 'easy-softonic-contact-form' );
			}
		}
        if ( empty($email) && self::$form_options['email_hide_empty'] == 'true' ) {

        } else {
                $this_label = (self::$form_options['title_email'] != '') ? self::$form_options['title_email'] : __( 'Email:', 'easy-softonic-contact-form' );
                self::$email_msg .= self::make_bold( $this_label ) . $inline_or_newline;
    ...
    static function validate_email_type( $slug, $req ) {
      ...
      if ( 'true' == $req  ) {
           if ( ! ESCF_Util::validate_email( self::$form_data[$slug] ) )
	       self::$form_errors[$slug] = (self::$form_options['error_email'] != '') ? self::$form_options['error_email'] : __( 'A proper email address is required.', 'easy-softonic-contact-form' );
	  } else if ( !empty(self::$form_data[$slug]) ) {
           if ( ! ESCF_Util::validate_email( self::$form_data[$slug] ) )  // was not required but something filled it, so ckeck
	       self::$form_errors[$slug] =  (self::$form_options['error_email_check'] != '') ? self::$form_options['error_email_check'] : __( 'Not a proper email address.', 'easy-softonic-contact-form' );
      }
     ...
     static function validate_url_type( $slug, $req ) {
      ...
      if ( 'true' == $req  ) {
           if ( ! ESCF_Util::validate_url( self::$form_data[$slug] ) )
	       self::$form_errors[$slug] = (self::$form_options['error_url'] != '') ? self::$form_options['error_url'] : __( 'Invalid URL.', 'easy-softonic-contact-form' );
	  } else if ( !empty(self::$form_data[$slug]) ) {
           if ( ! ESCF_Util::validate_url( self::$form_data[$slug] ) )  // was not required but something filled it, so ckeck
	       self::$form_errors[$slug] =  (self::$form_options['error_url'] != '') ? self::$form_options['error_url'] : __( 'Invalid URL.', 'easy-softonic-contact-form' );
      }
    ...
    static function validate_subject_select( $field ) {
      ...
      if( empty($sid) ) {
               self::$form_errors['subject'] = (self::$form_options['error_subject'] != '') ? self::$form_options['error_subject'] :  __('Selecting a subject is required.', 'easy-softonic-contact-form');
      } else if (empty($opts_array) || !isset($opts_array[$sid])) {
               self::$form_errors['subject'] = __('Requested subject not found.', 'easy-softonic-contact-form');
      } else {
    ...
    static function validate_select( $slug, $field ) {
      ...
      if ( 'true' == $field['req'] ) {
         if( empty($sid) ) {
                  self::$form_errors[$slug] = ( self::$form_options['error_field'] != '') ? self::$form_options['error_field'] : __('This field is required.', 'easy-softonic-contact-form');
         } else if (empty($opts_array) || !isset($opts_array[$sid])) {
                  self::$form_errors[$slug] = ( self::$form_options['error_field'] != '') ? self::$form_options['error_field'] : __('This field is required.', 'easy-softonic-contact-form');
         }
      }
	...
	static function validate_attach( $slug, $req, $label, $inline_or_newline ) {
		...
		if($field_file['name'] != '') {  // may not be required
			if ( self::$form_options['php_mailer_enable'] == 'php' ) {
				self::$form_errors[$slug] = __( 'Attachments not supported.', 'easy-softonic-contact-form' );
                return;
			} else if ( ($field_file['error'] && UPLOAD_ERR_NO_FILE != $field_file['error']) || !is_uploaded_file( $field_file['tmp_name'] ) ) {
				self::$form_errors[$slug] = __( 'Attachment upload failed.', 'easy-softonic-contact-form' );
                return;
				...
				if ( !preg_match( $file_type_pattern, $field_file['name'] ) ) {
					self::$form_errors[$slug] = __( 'Attachment file type not allowed.', 'easy-softonic-contact-form' );
					return;
				}
				...
				if ( $field_file['size'] > $allowed_size ) {
					self::$form_errors[$slug] = __( 'Attachment file size is too large.', 'easy-softonic-contact-form' );
					return;
				}
				...
				$filename = wp_unique_filename( ESCF_ATTACH_DIR, $filename );
				$new_file = trailingslashit( ESCF_ATTACH_DIR ) . $filename;

				if ( false === @move_uploaded_file( $field_file['tmp_name'], $new_file ) ) {
					self::$form_errors[$slug] = __( 'Attachment upload failed while moving file.', 'easy-softonic-contact-form' );
					return;
				}
				...
				self::$email_fields[$slug] =  __('File is attached:', 'easy-softonic-contact-form') . ' ' . $filename;
               ...
               if ( self::$form_options['email_hide_empty'] == 'true' ) {
                       ...
                       self::$email_msg .= self::make_bold( $label ) . $inline_or_newline;
                       self::$email_fields[$slug] =  __('No file attached', 'easy-softonic-contact-form');
			           self::$email_msg .= ' ' . __('No file attached', 'easy-softonic-contact-form') . self::$php_eol.self::$php_eol;
               }
        }
	...
	static function get_user_info() {
		...
		$eaysoftonic_web =  ' http://www.easysoftonic.com';
		$userdomain = '';
		...
		if ( $user_ID != '' ) {
			...
			if ( $current_user->user_login != '' )
				$user_info['wp_user'] = __( 'From a WordPress user', 'easy-softonic-contact-form' ) . ': ' . $current_user->user_login;
			if ( $current_user->user_email != '' )
			   	$user_info['wp_user_email'] = __( 'User email', 'easy-softonic-contact-form' ) . ': ' . $current_user->user_email;
			if ( $current_user->user_firstname != '' )
				$user_info['wp_user_first_name'] = __( 'User first name', 'easy-softonic-contact-form' ) . ': ' . $current_user->user_firstname;
			if ( $current_user->user_lastname != '' )
				$user_info['wp_user_last_name'] = __( 'User last name', 'easy-softonic-contact-form' ) . ': ' . $current_user->user_lastname;
			if ( $current_user->display_name != '' )
			    $user_info['wp_user_display_name'] = __( 'User display name', 'easy-softonic-contact-form' ) . ': ' . $current_user->display_name;
		}
		$user_info['wp_user_ip'] = __( 'Sent from (ip address)', 'easy-softonic-contact-form' ) . ': ' . esc_attr( $_SERVER['REMOTE_ADDR'] ) . " ($userdomain)";
		if ( $geo_loc != '' ) {
			$user_info['wp_user_location'] = __( 'Location', 'easy-softonic-contact-form' ) . ': ' . $geo_loc;
			self::$form_data['sender_location'] = __( 'Location', 'easy-softonic-contact-form' ) . ': ' . $geo_loc;
		}
	    $user_info['wp_user_date'] = __( 'Date/Time', 'easy-softonic-contact-form' ) . ': ' . date_i18n( get_option( 'date_format' ) . ' ' . get_option( 'time_format' ), current_time('timestamp') );
		$user_info['wp_user_referer'] = __( 'Plugin Website "'.$eaysoftonic_web.'"', 'easy-softonic-contact-form' ) . ': ' . esc_url( self::$form_action_url );
		$user_info['wp_user_agent'] = __( 'Using (user agent)', 'easy-softonic-contact-form' ) . ': ' . ESCF_Util::clean_input( $_SERVER['HTTP_USER_AGENT'] ) . self::$php_eol;

        // filter hook to allow modify $user_info array
        $user_info = apply_filters('es_contact_user_info', $user_info, self::$form_id_num);
	...
	static function check_captcha() {
		...
		if ( ESCF_Display::is_captcha_enabled(self::$form_id_num) ) {
			$captcha_code = ESCF_Util::clean_input( $_POST['captcha_code'] );
			...
			if ( self::$global_options['enable_php_sessions'] == 'true' ) { // this feature only works when PHP sessions are enabled
				...
				if ( !isset( $_SESSION['securimage_code_ctf_' . self::$form_id_num] ) || empty( $_SESSION['securimage_code_ctf_' . self::$form_id_num] ) ) {
					self::$form_errors['captcha'] = __( 'That CAPTCHA was incorrect. Try again.', 'easy-softonic-contact-form' );
				} else {
					if ( empty( $captcha_code ) ) {
						self::$form_errors['captcha'] = (self::$form_options['error_captcha_blank'] != '') ? self::$form_options['error_captcha_blank'] : __( 'Please complete the CAPTCHA.', 'easy-softonic-contact-form' );
					} else {
						require_once ESCF_CAPTCHA_PATH . '/securimage.php';
						...
						if ( $valid == true ) {
							...
							self::$form_errors['captcha'] = (self::$form_options['error_captcha_wrong'] != '') ? self::$form_options['error_captcha_wrong'] : __( 'That CAPTCHA was incorrect.', 'easy-softonic-contact-form' );
						}
					}
				}
			} else {
				...
				if ( empty( $captcha_code ) ) {
					self::$form_errors['captcha'] = (self::$form_options['error_captcha_blank'] != '') ? self::$form_options['error_captcha_blank'] : __( 'Please complete the CAPTCHA.', 'easy-softonic-contact-form' );
				} else if ( !isset( $_POST['escf_captcha_prefix' . self::$form_id_num] ) || empty( $_POST['escf_captcha_prefix' . self::$form_id_num] ) ) {
                    // this error means PHP session error, or they sat on the page more than 30 min
					self::$form_errors['captcha'] = __( 'That CAPTCHA was incorrect. Try again.', 'easy-softonic-contact-form' );
				} else {
					$prefix = 'xxxxxx';
					if ( isset( $_POST['escf_captcha_prefix' . self::$form_id_num] ) && is_string( $_POST['escf_captcha_prefix' . self::$form_id_num] ) && preg_match( '/^[a-zA-Z0-9]{15,17}$/', $_POST['escf_captcha_prefix' . self::$form_id_num] ) ) {
						$prefix = $_POST['escf_captcha_prefix' . self::$form_id_num];
					}
					if ( is_readable( ESCF_CAPTCHA_PATH . '/cache/' . $prefix . '.php' ) ) {
						include( ESCF_CAPTCHA_PATH . '/cache/' . $prefix . '.php' );
						// has the right CAPTCHA code has been entered?
						if ( 0 == strcasecmp( $captcha_code, $captcha_word ) ) {
							// captcha was matched
							@unlink( ESCF_CAPTCHA_PATH . '/cache/' . $prefix . '.php' );
							// ok can continue
						} else {
							self::$form_errors['captcha'] = (self::$form_options['error_captcha_wrong'] != '') ? self::$form_options['error_captcha_wrong'] : __( 'That CAPTCHA was incorrect.', 'easy-softonic-contact-form' );
						}
			...
			//$c['comment_author']  = "viagra-test-123";  // uncomment this to test spam detection
				...
				if ( 'true' == $response[1] ) {
					if ( self::$form_options['akismet_send_anyway'] == 'false' ) {
						self::$form_errors['akismet'] = (self::$form_options['error_input'] != '') ? self::$form_options['error_input'] : __( 'Invalid Input - Spam?', 'easy-softonic-contact-form' );
						global $user_ID;
						if ( $user_ID != '' && current_user_can( 'manage_options' ) ) {
                            // this error only shown to WP admins
							// show administrator a helpful message
							self::$form_errors['akismet'] .= '<br />' . __( 'Akismet determined your message is spam. This can be caused by the message content, your email address, or your IP address being on the Akismet spam system. The administrator can turn off Akismet for the form on the form edit menu.', 'easy-softonic-contact-form' );
						}
					} else {
						// Akismet says it is spam. flag the subject as spam and send anyway.
                        // XXX someday make these messages editable in settings
						self::$akismet_spam_subject = __( 'Akismet: Spam', 'easy-softonic-contact-form' ) . ' - ';
						$string .=  __( 'Akismet Spam Check: probably spam', 'easy-softonic-contact-form' ) . self::$php_eol;
						self::$email_fields['akismet'] = __( 'probably spam', 'easy-softonic-contact-form' );
					}
				} else {
					$string .= __( 'Akismet Spam Check: passed', 'easy-softonic-contact-form' ) . self::$php_eol;
					self::$email_fields['akismet'] = __( 'passed', 'easy-softonic-contact-form' );
				}
            }
		}
		return($string);
  ...
  static function vcita_create_or_validate_user($params, $global_defaults) {
    extract(self::vcita_post_contents("http://".$global_defaults['vcita_site']."/api/experts?".
                                       ...
                                       urlencode($params['vcita_last_name'])."&ref=wp-escf&o=int.1"));
  ...
  static function vcita_print_admin_page_notification($form_params, $global_options){
    if ($global_options['vcita_dismiss'] == 'false' && $form_params['vcita_scheduling_button'] == 'true' && $form_params['vcita_approved'] == 'false'){
      echo "<div id='si-escf-vcita-warning' class='esc-error'>Appointment booking by vCita has not been configured on the Scheduling tab yet&nbsp;<a href='".
      admin_url( 'plugins.php?page=easy-softonic-contact-form/easy-softonic-contact-form.php&escf_form='. ESCF_Options::$current_form)."&escf_tab=7'>Click to SETUP</a>, or <a href='#' onclick='document.getElementById(\"vcita_disable_button\").click();return false;'>Disable</a></div>";
    }
  ...
  static function vcita_disable_init_msg($form_params, $global_options) {
   if ($global_options['vcita_initialized'] == 'true'){
      ...
      $glob_options = ESCF_Util::get_global_options();
      $glob_options['vcita_initialized'] = 'false';
      update_option( 'es_contact_global', $glob_options );
    }
	...
	static function forbidifnewlines( $input ) {
		...
		if ( is_string( $input ) ) {
			...
			if ( preg_match( $input_expl, $v ) ) {
                // XXX someday make these messages editable in settings
				wp_die( __( 'Illegal characters in POST. Possible email injection attempt', 'easy-softonic-contact-form' ) );
			}
		}
	...
	static function spamcheckpost() {
		...
		if ( ! isset( $_SERVER['HTTP_USER_AGENT'] ) ) {
			return __( 'Invalid User Agent', 'easy-softonic-contact-form' );
		}

		// Make sure the form was indeed POST'ed:
		if ( ! $_SERVER['REQUEST_METHOD'] == "POST" ) {
			return __( 'Invalid POST', 'easy-softonic-contact-form' );
		}
		...
		if ( self::$form_options['domain_protect'] == 'true' ) {
			...
			if ( isset( $_SERVER['HTTP_REFERER'] ) and trim( $_SERVER['HTTP_REFERER'] ) != '' ) {
				...
				if ( ! in_array( $test_url , $domain_arr )  )
					return sprintf( __( 'Invalid HTTP_REFERER domain. The domain name posted from does not match the allowed domain names from the form edit Security tab: %s', 'easy-softonic-contact-form' ), $print_authHosts );
			}
		...
		foreach ( $_POST as $k => $v ) {
			if ( is_string( $v ) ) {
				...
				if ( preg_match( $input_expl, $v ) ) {
                  // XXX someday make these messages editable in settings
					return __( 'Illegal characters in POST. Possible email injection attempt', 'easy-softonic-contact-form' );
				}
			}
		}
	...
	static function prepare_email() {
		...
		if ( self::$form_options['silent_send'] != 'off' && self::$form_options['silent_email_off'] == 'true' )
            ...
            self::$email_fields['from_name'] = apply_filters('es_contact_from_name', self::$email_fields['from_name'], self::$form_id_num);
            self::$email_fields['from_email'] = apply_filters('es_contact_from_email', self::$email_fields['from_email'], self::$form_id_num);
		...
		if ( ! $email_off ) {
           if ( $no_name && self::$email_header['from_name'] == '~' )  // they had no name field
			...
			$posted_form_name = ( self::$form_options['form_name'] != '' ) ? self::$form_options['form_name'] : sprintf( __( 'Form: %d', 'easy-softonic-contact-form' ), self::$form_id_num );
            ...
            $subj = apply_filters('es_contact_email_subject', $subj, self::$form_id_num);
            ...
            $header  = apply_filters('es_contact_email_header', $header, self::$form_id_num);
          ...
          /*  print $header;
			...
			if ( self::$form_options['php_mailer_enable'] == 'php' ) {
                ...
                add_filter( 'wp_mail_from_name', 'ESCF_Process::set_wp_from_name', 1 );

                self::$email_set_wp['from_email'] = self::$email_header['from_email'];
				add_filter( 'wp_mail_from', 'ESCF_Process::set_wp_from_email', 1 );

				if ( $ctf_email_on_this_domain != '' ) {
					// Add an action on phpmailer_init to add Sender $this->es_contact_mail_sender for Return-path in wp_mail
					...
					add_action( 'phpmailer_init', 'ESCF_Process::set_wp_mail_sender', 1 );
				}
                ...
                $send_attachments= apply_filters( 'es_contact_send_attachments', $send_attachments,  self::$form_id_num);
        ...
        !empty(self::$form_options['auto_respond_message']) ) {
            ...
            $subj = apply_filters('es_contact_autoresp_email_subject', $subj, self::$form_id_num);
       ...
       if ( !empty(self::$form_options['silent_conditional_field']) && !empty(self::$form_options['silent_conditional_value']) ) {
        ...
        self::$email_fields = apply_filters('es_contact_email_fields_posted', self::$email_fields, self::$form_id_num);
	...
	static function email_sent_redirect() {
		...
		if ( self::$redirect_enable == 'true' ) {
            ...
            $ctf_redirect_url = apply_filters( 'es_contact_redirect_url', $ctf_redirect_url, self::$email_fields,  self::$form_data['mailto_id'], self::$form_id_num);
			...
			if (is_admin())
				add_action('admin_head', 'ESCF_Process::meta_refresh',1);
			else
				add_action('wp_head', 'ESCF_Process::meta_refresh',1);

...
/**
 * Description of class-escf-display
...
class ESCF_Display {
	...
	static $have_attach = '';
	static $printed_tooltip_filetypes, $escf_use_window_onload;
	static $add_escf_script, $add_placeholder_script, $add_date_js_array, $add_date_js;

	static function process_short_code($atts) {
		// Process shortcode and display the form
		// and decide whether to send the email or not

        self::$add_escf_script = true;  // condition flag for add styles and scripts to page or post the form is on
		...
		if ( isset($form) && is_numeric($form) ) {
           ...
           echo __( 'Contact Form Shortcode Error: Invalid form number in shortcode.', 'easy-softonic-contact-form' );
           return;
        }
		...
		self::$form_options = ESCF_Util::get_form_options( self::$form_id_num, false );  // Don't use defaults if it doesn't exist

		if ( ! self::$form_options ) {
			// Form does not exist in options table
			// Display error message and return
            echo  sprintf( __( 'Contact Form Shortcode Error: Form %s does not exist', 'easy-softonic-contact-form' ), self::$form_id_num );
            return;
		}
		...
		// http://www.EasySoftonicContactForm.com/shortcode-options
        if (self::$global_options['enable_php_sessions'] == 'true') { // this feature only works when PHP sessions are enabled
           ...
           if ( !empty($redirect) || !empty($hidden) || !empty($email_to)) {
            ...
            // Display error message and return
            echo  __( 'Contact Form Shortcode Error: Using shorcode attributes requires the PHP sessions setting to be enabled on the Advanced tab in form settings.', 'easy-softonic-contact-form' );
            return;
           }
        }
		...
		if ( ESCF_Process::$form_processed && ESCF_Process::$form_id_num == self::$form_id_num  && empty(ESCF_Process::$form_errors) ) {
			...
			if ( ! empty(ESCF_Process::$form_errors) && ESCF_Process::$form_id_num == self::$form_id_num ) {
				// The form was processed, but had errors
				if ( ! empty(ESCF_Process::$form_data) ) {
					// If this is not true, there is an internal error...
					self::$form_content = array_merge( self::$form_content, ESCF_Process::$form_data );
					self::$form_errors  = array_merge( self::$form_errors, ESCF_Process::$form_errors );
			...
			// ***** Display the Form *****
            self::$placeholder = 0;
            $string = "\n\n<!-- Easy Softonic Contact Form plugin " . ESCF_VERSION . " - begin - EasySoftonicContactForm.com -->
...
<?php
  ...
  static function get_ext_css() {
   ...
   self::$ext_css = array(
            ...
            // Alignment DIVs
		    'form_style'           => 'escf-div-form',   // Form DIV, how wide is the form DIV
            'left_box_style'       => 'escf-div-left-box',   // left box DIV, container for vcita
            'right_box_style'      => 'escf-div-right-box',   // right box DIV, container for vcita
		    'clear_style'          => 'escf-div-clear',   // clear both
		    'field_left_style'     => 'escf-div-field-left',   // field left
            'field_prefollow_style' => 'escf-div-field-prefollow',   // field prefollow
		    'field_follow_style'   => 'escf-div-field-follow',   // field follow

			'title_style'          => 'escf-div-label', // Input labels alignment DIV
			'field_div_style'      => 'escf-div-field',   // Input fields alignment DIV
			'captcha_div_style_sm' => 'escf-div-captcha-sm',  // Small CAPTCHA DIV
			'captcha_div_style_m'  => 'escf-div-captcha-m',  // Large CAPTCHA DIV
			'captcha_image_style'  => 'escf-image-captcha', // CAPTCHA image alignment
			'captcha_reload_image_style' => 'escf-image-captcha-refresh', // CAPTCHA refresh image alignment
			'submit_div_style'     => 'escf-div-submit', // Submit DIV
            'border_style'         => 'escf-fieldset', // style of the fieldset box (if enabled)

		    // Styles of labels, fields and text

            'required_style'       => 'escf-required-indicator',   // required field indicator
            'required_text_style'  => 'escf-required-text',   // required field text
			'hint_style'           => 'escf-hint-text',  // small text hints like file types
            'error_style'          => 'escf-div-error', // Input validation messages
            'redirect_style'       => 'escf-div-redirecting', // Redirecting message
            'fieldset_style'       => 'escf-fieldset-field', // style of the fieldset box (for a field)

            'label_style'          => 'escf-label', // Field labels
  			'option_label_style'   => 'escf-option-label', // Options labels

 			'field_style'          => 'escf-input-text', // Input text fields
  			'captcha_input_style'  => 'escf-input-captcha', // CAPTCHA input field
 			'textarea_style'       => 'escf-input-textarea',  // Input Textarea
            'select_style'         => 'escf-input-select',  //  Input Select
 			'checkbox_style'       => 'escf-input-checkbox',  // Input checkbox
            'radio_style'          => 'escf-input-radio',  // Input radio

			'button_style'         => 'escf-button-submit', // Submit button
			'reset_style'          => 'escf-button-reset', // Reset button
            'vcita_button_style'   => 'escf-button-vcita', // vCita button
            'vcita_div_button_style' => 'escf-button-div-vcita', // vCita button div box

 			'powered_by_style'     => 'escf-powered-by', // the "powered by" link
  ...
  // filter a list of field names that are not currently used on the form
	...
	static function display_form($string) {
		...
		$captcha_path_cf = ESCF_CAPTCHA_PATH;
...
<div id="FSContact' . self::$form_id_num . '" ' . self::get_this_css('form_style') . '>';
        ...
        if (self::$form_options['vcita_scheduling_button'] == 'true' && self::is_vcita_activated() )
	         $string .= "\n<div ".'id="escf_div_left_box' . self::$form_id_num . '" '.self::get_this_css('left_box_style').">";
...
$string .= '
<form ' . self::$have_attach . 'action="' . esc_url( self::$form_action_url ) . $anchor . '" id="escf_form' . self::$form_id_num . '" '.$form_attributes.'method="post">
';

		if ( self::$form_options['border_enable'] == 'true' ) {
			$string .=	'<fieldset id="escf_form_fieldset' . self::$form_id_num . '" ' . self::get_this_css('border_style') . ">\n";
		...
		if ( self::$have_attach ) {
			self::init_temp_dir( ESCF_ATTACH_DIR );
			if ( self::$form_options['php_mailer_enable'] == 'php' ) {
				self::set_form_error("escf_attach_dir$frm_id", __( 'Attachments are only supported when the Send Email function is set to WordPress. You can find this setting on the contact form settings page.', 'easy-softonic-contact-form' ) );
			}
			if ( ! is_dir( ESCF_ATTACH_DIR ) ) {
				self::set_form_error("escf_attach_dir$frm_id", __( 'The temporary folder for the attachment field does not exist.', 'easy-softonic-contact-form' ) );
			} else if ( ! is_writable( ESCF_ATTACH_DIR ) ) {
				self::set_form_error("escf_attach_dir$frm_id", __( 'The temporary folder for the attachment field is not writable.', 'easy-softonic-contact-form' ) );
			} else {
				// delete files over 3 minutes old in the attachment directory
                // full directory sweep cleanup
				//self::clean_temp_dir( ESCF_ATTACH_DIR, 3 );
			}
		}
      ...
      if ( $post_types ) {
        ...
        foreach ($post_types_slugs as $key => $slug) {
            if ( in_array( strtolower( $slug ), $slug_list ) ) {
             	$string .= '
		<div id="escf_form_error_email' . self::$form_id_num . '" ' . self::get_this_css('error_style') . '>' .sprintf( __( 'Warning: one of your field tags conflicts with the post type redirect tag "%s". To automatically correct this, click the <b>Save Changes</b> button on the form edit page.', 'easy-softonic-contact-form' ), $slug )
         . "\n    </div>\n";
            }
		...
		if ( self::$contact_error ) {
			...
			$string .= '      <div id="escf_form_error' . self::$form_id_num . '" ' . self::get_this_css('error_style') . ">\n";
			$string .= (self::$form_options['error_correct'] != '') ? self::$form_options['error_correct'] : __( 'Please make corrections below and try again.', 'easy-softonic-contact-form' );
			$string .= "\n    </div>\n";
			
			// Print errors that appear at the top of the form
			$string .= self::echo_if_error("escf_attach_dir$frm_id");
		...
		if ( empty( self::$contacts ) ) {	// was $ctf_contacts
			$string .= '
		<div id="escf_form_error_email' . self::$form_id_num . '" ' . self::get_this_css('error_style') . '>' . __( 'ERROR: Misconfigured email address in options.', 'easy-softonic-contact-form' )
         . "\n    </div>\n";
		}
		...
		if ( is_admin() )
			$hidden .= '<input type="hidden" name="ctf_action" value="' . __('Preview Form', 'easy-softonic-contact-form') . '" />'. "\n";

		$hidden .= '<input type="hidden" name="escf_submitted" value="0" />'. "\n";
		$hidden .= '<input type="hidden" name="es_postonce_'.self::$form_id_num.'" value="'. wp_hash( time() ).','.time() .'" />'. "\n";
		$hidden .= '<input type="hidden" name="es_contact_action" value="send" />'. "\n";
		...
		if ( self::$form_options['req_field_label_enable'] == 'true' && self::$form_options['req_field_indicator_enable'] == 'true' ) {
            $string .= "\n".'<div id="escf_required'.self::$form_id_num.'">' . "\n";
            ...
            $string .= (self::$form_options['tooltip_required'] != '') ? self::$form_options['tooltip_required'] : __( 'indicates required field', 'easy-softonic-contact-form' );
		    $string .= "</span>\n</div>\n\n";
		}
		// If there are multiple mail-to contacts, display a select form
		if ( count( self::$contacts ) > 1 ) {

			$string .= '<div id="escf_div_clear_contact' . self::$form_id_num.'" '.self::get_this_css('clear_style').'>' . "\n" . '  <div id="escf_div_field_contact' . self::$form_id_num . '" '.self::get_this_css('field_left_style').'>
    <div ' . self::get_this_css('title_style') . '>
      <label '.self::get_this_css('label_style').' for="escf_mail_to' . self::$form_id_num . '">';
			$string .= (self::$form_options['title_dept'] != '') ? self::$form_options['title_dept'] : __( 'Select a contact:', 'easy-softonic-contact-form' );
			$string .= self::$req_field_ind . '</label>
    </div>
    <div ' . self::get_this_css('field_div_style') . '>' . self::echo_if_error( 'contact' ) . '
      <select ' . self::get_this_css('select_style') . ' id="escf_mail_to' . self::$form_id_num . '" name="mailto_id" ' . self::$aria_required . '>
';
			$string .= '       <option value="">';
			$string .= (self::$form_options['title_select'] != '') ? esc_html( self::$form_options['title_select'] ) : esc_html( __( 'Select', 'easy-softonic-contact-form' ) );
			$string .= "</option>\n";
...
';
		...
		// A div class="escf-clear" is used to group a field with any that follow it
		...
		foreach ( self::$form_options['fields'] as $key => $field ) {
			...
			if ( 'true' == $field['follow'] && $open_div ) {
				$string .= '  <div id="escf_div_follow' . self::$form_id_num.'_'.$key.'" '.self::get_this_css('field_follow_style').'>';
			} else {
				if ( $open_div ) {
				...
				if ( 'fieldset' == $field['type'] || 'fieldset-close' == $field['type'] ) {
					...
					$open_div = true;
					$string .= "\n" . '<div id="escf_div_clear' . self::$form_id_num.'_'.$key.'" '.self::get_this_css('clear_style').'>' . "\n" . '  ';
                    $string .= '<div id="escf_div_field' . self::$form_id_num.'_'.$key.'" ';
...
//                    if ( ( isset(self::$form_options['fields'][$key+1] ) && self::$form_options['fields'][$key+1]['follow'] == 'true' )
                    if ( ( isset($field_follow[$fld_cnt+1] ) && $field_follow[$fld_cnt+1] == 'true' )
						// Added by Umair Saleem for displaying name parts inline
						|| ( ESCF_NAME_FIELD == $field['standard'] && 'name' != self::$form_options['name_format'] && 'true' == $field['inline'] )
			...
			if ( ! in_array( $field['type'], array('fieldset', 'fieldset-close', 'hidden') ) ) {
				...
				if ( ('checkbox' != $field['type'] && $field['standard'] < 1) && ( 'false' == $field['hide_label'] ) ) {
					...
					$string .= "\n    <div ".' id="escf_label' . self::$form_id_num . '_' . $key . '" ' .  self::get_this_css('title_style') .'>
      <label ';
	    $string .= ( $field['label_css'] != '') ? self::convert_css( $field['label_css'] ) : self::get_this_css('label_style');
		$string .= ' for="escf_field' . self::$form_id_num . '_' . $key . '">' . esc_html($field['label']);
			...
			switch ( $field['type'] ) {
				case 'fieldset' :
					...
					$string .= "\n<fieldset".' id="escf_fieldset' . self::$form_id_num . '_' . $key . '" ';
...
';
	...
	$string .= '	  <legend>' . esc_html($field['label']) . "</legend>\n";
		...
		$string .= ' type="password" id="escf_field'.self::$form_id_num.'_'.$key.'" name="' . $field['slug'] . '" value=""'
//					No defaults for password!
        ...
        $string = apply_filters( 'es_contact_display_after_fields', $string, self::$style, self::$form_errors, self::$form_id_num );
...
<div style="display:none;">
     <label for="'.$honeypot_slug.self::$form_id_num.'"><small>'.__('Leave this field empty', 'easy-softonic-contact-form').'</small></label>
...
';
		...
		$string .= "\n<div ".'id="escf_submit_div'.self::$form_id_num.'" ' . self::get_this_css('submit_div_style') . '>
		<input type="submit" id="escf_submit' . self::$form_id_num . '" ' . self::get_this_css('button_style') . ' value="';
		$string .= (self::$form_options['title_submit'] != '') ? esc_attr( self::$form_options['title_submit'] ) : esc_attr( __( 'Submit', 'easy-softonic-contact-form' ) );
		$string .= '" ';
        $onclick = 0;
		...
		if ( self::$form_options['enable_areyousure'] == 'true' && !$onclick) {
			$msg = (self::$form_options['title_areyousure'] != '') ? esc_html( addslashes( self::$form_options['title_areyousure'] ) ) : esc_html( addslashes( __( 'Are you sure?', 'easy-softonic-contact-form' ) ) );
        ...
        if( self::$form_options['enable_submit_oneclick'] == 'true' && self::$form_options['enable_areyousure'] != 'true' && !$onclick) {
          $msg = (self::$form_options['title_submitting'] != '') ? esc_html( addslashes( self::$form_options['title_submitting'] ) ) : esc_html( addslashes( __( 'Submitting...', 'easy-softonic-contact-form' ) ) );
		...
		if ( !self::$contact_error && self::$form_options['enable_reset'] == 'true' ) {
			$string .= '<input type="reset" id="escf_reset' . self::$form_id_num . '" ' . self::get_this_css('reset_style') . ' value="';
			$string .= (self::$form_options['title_reset'] != '') ? esc_attr( self::$form_options['title_reset'] ) : esc_attr( __( 'Reset', 'easy-softonic-contact-form' ) );
			$msg = addslashes( __( 'Do you really want to reset the form?', 'easy-softonic-contact-form' ) );
			$string .= '" onclick="return confirm(\'' . $msg . '\')" />';
		}
		...
		if ( self::$form_options['enable_credit_link'] == 'true' ) {
			$string .= "\n    <p " . self::convert_css( self::$form_options['powered_by_style'] ) . '>' . __( 'Powered by', 'easy-softonic-contact-form' )
				. ' <a href="http://wordpress.org/extend/plugins/easy-softonic-contact-form/" target="_blank">' . __( 'Easy Softonic Contact Form', 'easy-softonic-contact-form' )
				. "</a></p>\n";
		}

        if ( self::$form_options['vcita_scheduling_button'] == 'true' && self::is_vcita_activated() ) {
           $string .= "</div>\n<div ".'id="escf_div_right_box' . self::$form_id_num . '" '.self::get_this_css('right_box_style').">\n";
		   $string = self::display_vcita_scheduler_button( $string );
		   $string .= "\n</div>\n";
        }


		$string .= '</div>';	// closes escf-container
		$string .= "
<div ".self::get_this_css('clear_style')."></div>\n";
...
';
		...
		$string .= "\n".'<!-- Easy Softonic Contact Form plugin '.ESCF_VERSION.' - end - EasySoftonicContactForm.com -->'. "\n";
...
$f_name_string .= '      <label ';
		...
		$f_name_string .= ' for="escf_f_name' . self::$form_id_num . '">';
		$f_name_string .= (self::$form_options['title_fname'] != '') ? self::$form_options['title_fname'] : __( 'First Name:', 'easy-softonic-contact-form' );
                if ( 'true' == $field['req'] )
...
$f_name_string .= '    </div>
    <div ' . self::get_this_css('field_div_style') . '>' . self::echo_if_error( 'f_name' ) . '
	  <input ';
		...
		$f_name_string .= ' type="text" id="escf_f_name' . self::$form_id_num .
...
$l_name_string .= '      <label ';
		...
		$l_name_string .= ' for="escf_l_name' . self::$form_id_num . '">';
		$l_name_string .= (self::$form_options['title_lname'] != '') ? self::$form_options['title_lname'] : __( 'Last Name:', 'easy-softonic-contact-form' );
                if ( 'true' == $field['req'] )
...
$l_name_string .= '    </div>
    <div ' . self::get_this_css('field_div_style') . '>' . self::echo_if_error( 'l_name' ) . '
	  <input ';
		...
		$l_name_string .= ' type="text" id="escf_l_name' . self::$form_id_num . '" name="l_name" value="' . esc_attr( self::$form_content['l_name'] ) . '" ' . self::$aria_required;
        if($field['attributes'] != '')
...
$string .= '      <label ';
	    $string .= ( $field['label_css'] != '') ? self::convert_css( $field['label_css'] ) : self::get_this_css('label_style');
		$string .= ' for="escf_name' . self::$form_id_num . '">';
				$string .= (self::$form_options['title_name'] != '') ? self::$form_options['title_name'] : __( 'Name:', 'easy-softonic-contact-form' );
                if ( 'true' == $field['req'] )
...
$string .= '    </div>
    <div ' . self::get_this_css('field_div_style') . '>' . self::echo_if_error( 'full_name' ) . '
      <input ';
		$string .= ($field['input_css'] != '') ? self::convert_css( $field['input_css'] ) : self::get_this_css('field_style');
		$string .= ' type="text" id="escf_name' . self::$form_id_num . '" name="full_name" value="' . esc_attr( self::$form_content[$field['slug']] ) . '" ' . self::$aria_required;
        if($field['attributes'] != '')
			  $string .= ' '.$field['attributes'];
...
$string .= '      <label ';
	    $string .= ( $field['label_css'] != '') ? self::convert_css( $field['label_css'] ) : self::get_this_css('label_style');
		$string .= ' for="escf_mi_name' . self::$form_id_num . '">';
				$string .= (self::$form_options['title_miname'] != '') ? self::$form_options['title_miname'] : __( 'Middle Initial:', 'easy-softonic-contact-form' );
				$string .= "</label>\n";
              }
$string .= '    </div>
    <div ' . self::get_this_css('field_div_style') . '>' . self::echo_if_error( 'mi_name' ) . '
       <input ';
		$string .= ($field['input_css'] != '') ? self::convert_css( $field['input_css'] ) : self::get_this_css('field_style');
		$string .= ' type="text" id="escf_mi_name' . self::$form_id_num . '" name="mi_name" value="' . esc_attr( self::$form_content['mi_name'] ) . '" ';
        if($field['attributes'] != '')
		     $string .= ' '.$field['attributes'];
...
$string .= '      <label ';
	    $string .= ( $field['label_css'] != '') ? self::convert_css( $field['label_css'] ) : self::get_this_css('label_style');
		$string .= ' for="escf_m_name' . self::$form_id_num . '">';
				$string .= (self::$form_options['title_mname'] != '') ? self::$form_options['title_mname'] : __( 'Middle Name:', 'easy-softonic-contact-form' );
				$string .= "</label>\n";
              }
$string .= '    </div>
    <div ' . self::get_this_css('field_div_style') . '>' . self::echo_if_error( 'm_name' ) . '
      <input ';
		$string .= ($field['input_css'] != '') ? self::convert_css( $field['input_css'] ) : self::get_this_css('field_style');
		$string .= ' type="text" id="escf_m_name' . self::$form_id_num . '" name="m_name" value="' . esc_attr( self::$form_content['m_name'] ) . '" ' . self::$aria_required;
        if($field['attributes'] != '')
			  $string .= ' '.$field['attributes'];
 if ( 'true' == $field['placeholder'] && $m_default != '') {
	...
	static function display_email($key, $field) {
        ...
        $email_input_type = apply_filters( 'es_contact_email_input_type', 'text',  self::$form_id_num);
...
$string .= '      <label ';
	    $string .= ( $field['label_css'] != '') ? self::convert_css( $field['label_css'] ) : self::get_this_css('label_style');
		$string .= ' for="escf_email' . self::$form_id_num . '">';
			$string .= (self::$form_options['title_email'] != '') ? self::$form_options['title_email'] : __( 'Email:', 'easy-softonic-contact-form' );
                if ( 'true' == $field['req'] )
					$string .= self::$req_field_ind;
                $string .= "</label>\n";
              }
        ...
        $string .= '    <div ' . self::get_this_css('field_div_style') . '>' . self::echo_if_error( 'email' )
			. "\n      <input ";
		$string .= ($field['input_css'] != '') ? self::convert_css( $field['input_css'] ) : self::get_this_css('field_style');
		$string .= ' type="'.$email_input_type.'" id="escf_email' . self::$form_id_num . '" name="email" value="';
		...
		if ( 'true' == self::$form_options['double_email'] ) {
            $string .= "  </div>\n</div>\n\n" . '<div id="escf_div_clear' . self::$form_id_num.'_'.$key.'_2" '. self::get_this_css('clear_style') .'>' . "\n" . '  <div id="escf_div_field' . self::$form_id_num.'_'.$key.'_2" '. self::get_this_css('field_left_style') .'>';
            $string .= "\n    <div " . self::get_this_css('title_style') .  ">\n";
                if ( 'false' == $field['hide_label'] ) {
$string .= '      <label ';
	    $string .= ( $field['label_css'] != '') ? self::convert_css( $field['label_css'] ) : self::get_this_css('label_style');
		$string .= ' for="escf_email' . self::$form_id_num . '_2">';
			$string .= (self::$form_options['title_email2'] != '') ? self::$form_options['title_email2'] : __( 'Re-enter Email:', 'easy-softonic-contact-form' );
                if ( 'true' == $field['req'] )
					$string .= self::$req_field_ind;
                $string .= "</label>\n";
              }
        $string .= "    </div>\n    <div " . self::get_this_css('field_div_style') . '>' . self::echo_if_error( 'email2' )
                . "\n      <input ";
		$string .= ($field['input_css'] != '') ? self::convert_css( $field['input_css'] ) : self::get_this_css('field_style');
		$string .= ' type="'.$email_input_type.'" id="escf_email' . self::$form_id_num. '_2" name="email2" value="' . esc_attr( self::$form_content['email2'] ) . '" ' . self::$aria_required;
  ...
  //$string .= ' onfocus="if(this.value==\''.esc_js($email2_default).'\')this.value=\'\';" onblur="if(this.value==\'\')this.value=\''.esc_js($email2_default).'\';"';
	...
	static function display_field_text($key, $field) {
		...
		$string = '';
		if ( ESCF_NAME_FIELD == $field['standard'] ) return(self::display_name($key, $field));
		if ( ESCF_EMAIL_FIELD == $field['standard'] ) return(self::display_email($key, $field));

		if ( ESCF_SUBJECT_FIELD == $field['standard']) {
...
$string .= '      <label ';
	    $string .= ( $field['label_css'] != '') ? self::convert_css( $field['label_css'] ) : self::get_this_css('label_style');
		$string .= ' for="escf_field'.self::$form_id_num.'_'.$key. '">';
			$string .= (self::$form_options['title_subj'] != '') ? self::$form_options['title_subj'] : __( 'Subject:', 'easy-softonic-contact-form' );
                if ( 'true' == $field['req'] )
					$string .= self::$req_field_ind;
                $string .= "</label>\n";
              }
$string .= "    </div>";
		...
		. '      <input ';
		$string .= ($field['input_css'] != '') ? self::convert_css( $field['input_css'] ) : self::get_this_css('field_style');
		$string .= ' type="' . $type . '" id="escf_field' . self::$form_id_num . '_' . $key . '" name="' . $field['slug'] . '" value="';
	...
	static function display_field_textarea($key, $field) {
		...
		if ( ESCF_MESSAGE_FIELD == $field['standard'] ) {
...
$string .= '      <label ';
	    $string .= ( $field['label_css'] != '') ? self::convert_css( $field['label_css'] ) : self::get_this_css('label_style');
		$string .= ' for="escf_field'.self::$form_id_num.'_'.$key. '">';
			$string .= (self::$form_options['title_mess'] != '') ? self::$form_options['title_mess'] : __( 'Message:', 'easy-softonic-contact-form' );
                if ( 'true' == $field['req'] )
					$string .= self::$req_field_ind;
                $string .= "</label>\n";
              }
$string .= "    </div>\n    <div " . self::get_this_css('field_div_style') . '>'
			...
			$string .= ' id="escf_field'.self::$form_id_num.'_'.$key . '" name="message" cols="30" rows="10" ' . self::$aria_required;
	        if($field['attributes'] != '')
					  $string .= ' '.$field['attributes'];
             if ( 'true' == $field['placeholder'] && $field['default'] != '') {
                $string .= ' placeholder="'.esc_attr($field['default']).'"';
			...
			$string	.= "\n    <div " . self::get_this_css('field_div_style') . '>' . self::echo_if_error( $field['slug'] ) . "\n"
				.'      <textarea ';
			$string .= ($field['input_css'] != '') ? self::convert_css( $field['input_css'] ) : self::get_this_css('textarea_style');
			$string .= ' id="escf_field' . self::$form_id_num . '_' . $key . '" name="' . $field['slug'] . '" cols="30" rows="10" ' . self::$aria_required;
			if ( $field['attributes'] != '' )
				$string .= ' ' . $field['attributes'];
              if ( 'true' == $field['placeholder'] && $field['default'] != '') {
                 $string .= ' placeholder="'.esc_attr($field['default']).'"';
	...
	static function display_field_select($key, $field) {
		...
		if ( '' == $opts_array[0] ) {
			...
			self::set_form_error("escf_select$frm_id", __('Error: No options were entered for a select field in settings.', 'easy-softonic-contact-form'));
			$string .= self::echo_if_error("escf_select$frm_id");
		}
...
$string .= "    </div>";
		...
		$string .= ' id="escf_field'.self::$form_id_num.'_'.$key.'" name="' . $field['slug'].'[]"';
		if($field['attributes'] != '')
			$string .= ' '.$field['attributes'];
		$string .= $mult .">\n";
	...
	static function display_field_checkbox($key, $field) {
		...
		if ( '' == $opts_array[0] ) {
			if ( 'checkbox' == $field['type']) {
				...
				self::$form_errors['escf_checkbox'] = __('Error: No options were entered for a checkbox-multiple field in settings.', 'easy-softonic-contact-form');
				$string .= self::echo_if_error('escf_checkbox');
			}
		}
		if ( 'checkbox' == $field['type'] ) {
			// Single checkbox
		...
		$string .= ' id="escf_field' . self::$form_id_num . '_'
		...
		$string .= ' id="escf_field'
			...
			if ( $field['attributes'] != '' )
				...
				$string .= ' /> <label ';
	    $string .= ( $field['label_css'] != '') ? self::convert_css( $field['label_css'] ) : self::get_this_css('option_label_style');
		$string .= ' for="escf_field' . self::$form_id_num . '_' . $key . '_'
	...
	static function display_field_date($key, $field) {
        ...
        $string .= "\n    <div " . self::get_this_css('field_div_style') . '>' . self::echo_if_error( $field['slug'] ) .
			"\n      <input ";
		$string .= ($field['input_css'] != '') ? self::convert_css( $field['input_css'] ) : self::get_this_css('field_style');
		$string .= ' type="text" id="escf_field' . self::$form_id_num . '_' . $key . '" name="' . $field['slug'] . '" value="';
	...
	static function display_field_attachment($key, $field) {
		...
		if ( self::$form_options['php_mailer_enable'] != 'php' ) {
			$string .= "\n    <div " . self::get_this_css('field_div_style') . '>' . self::echo_if_error( $field['slug'] )
				. "\n      <input " . self::get_this_css('field_style') . ' type="file" id="escf_field'
			...
			if ( ! self::$printed_tooltip_filetypes) {
				$file_type_message = "<br />\n      <span ".'id="escf_hint_file_types' . self::$form_id_num.'_'.$key.'" '.self::get_this_css('hint_style').'>';
				$file_type_message .= sprintf( (self::$form_options['tooltip_filetypes'] != '') ? self::$form_options['tooltip_filetypes'] : __( 'Acceptable file types: %s.', 'easy-softonic-contact-form' ), self::$form_options['attach_types'] );
                $file_type_message .= '<br />';
		        $file_type_message .= sprintf( (self::$form_options['tooltip_filesize'] != '') ? self::$form_options['tooltip_filesize'] :  __( 'Maximum file size: %s.', 'easy-softonic-contact-form' ), self::$form_options['attach_size']) . "</span>\n";
                //filter hook for file attachment acceptable types message
                $file_type_message = apply_filters( 'es_contact_file_type_message', $file_type_message, self::$form_options,  self::$form_id_num);
	...
	static function setup_calendar($date_fields) {
	   ...
	   if	( self::$add_date_js == '' ) { // only add for 1st form with date fields
		self::$add_date_js = '
<!-- Easy Softonic Contact Form plugin - begin date field js - form '.self::$form_id_num.' -->
<script type="text/javascript">
  var ctf_daylist = new Array( \'' . __( 'Su', 'easy-softonic-contact-form' ) . '\',\'' . __( 'Mo', 'easy-softonic-contact-form' ) . '\',\'' . __( 'Tu', 'easy-softonic-contact-form' ) . '\',\'' . __( 'We', 'easy-softonic-contact-form' ) . '\',\'' . __( 'Th', 'easy-softonic-contact-form' ) . '\',\'' . __( 'Fr', 'easy-softonic-contact-form' ) . '\',\'' . __( 'Sa', 'easy-softonic-contact-form' ) . '\',\'' . __( 'Su', 'easy-softonic-contact-form' ) . '\',\'' . __( 'Mo', 'easy-softonic-contact-form' ) . '\',\'' . __( 'Tu', 'easy-softonic-contact-form' ) . '\',\'' . __( 'We', 'easy-softonic-contact-form' ) . '\',\'' . __( 'Th', 'easy-softonic-contact-form' ) . '\',\'' . __( 'Fr', 'easy-softonic-contact-form' ) . '\',\'' . __( 'Sa', 'easy-softonic-contact-form' ) . '\' );
  var ctf_months_sh = new Array( \'' . __( 'Jan', 'easy-softonic-contact-form' ) . '\',\'' . __( 'Feb', 'easy-softonic-contact-form' ) . '\',\'' . __( 'Mar', 'easy-softonic-contact-form' ) . '\',\'' . __( 'Apr', 'easy-softonic-contact-form' ) . '\',\'' . __( 'May', 'easy-softonic-contact-form' ) . '\',\'' . __( 'Jun', 'easy-softonic-contact-form' ) . '\',\'' . __( 'Jul', 'easy-softonic-contact-form' ) . '\',\'' . __( 'Aug', 'easy-softonic-contact-form' ) . '\',\'' . __( 'Sep', 'easy-softonic-contact-form' ) . '\',\'' . __( 'Oct', 'easy-softonic-contact-form' ) . '\',\'' . __( 'Nov', 'easy-softonic-contact-form' ) . '\',\'' . __( 'Dec', 'easy-softonic-contact-form' ) . '\' );
  var ctf_monthup_title = \'' . __( 'Go to the next month', 'easy-softonic-contact-form' ) . '\';
  var ctf_monthdn_title = \'' . __( 'Go to the previous month', 'easy-softonic-contact-form' ) . '\';
  var ctf_clearbtn_caption = \'' . __( 'Clear', 'easy-softonic-contact-form' ) . '\';
  var ctf_clearbtn_title = \'' . __( 'Clears any dates selected on the calendar', 'easy-softonic-contact-form' ) . '\';
  var ctf_maxrange_caption = \'' . __( 'This is the maximum range', 'easy-softonic-contact-form' ) . '\';
  ...
  var ctf_date_format = \'';
		...
		self::$add_date_js .= '\';
';
        self::$escf_use_window_onload = true;
        //filter hook to suppress window.onload = function(){} for sites where it can only happen once per page.
        self::$escf_use_window_onload = apply_filters( 'es_contact_use_window_onload', self::$escf_use_window_onload,  self::$form_id_num);
        }
	...
	static function display_field_time($key, $field) {
		...
		$string .= "      </select>:\n      <select ";
		$string .= ($field['input_css'] != '') ? self::convert_css( $field['input_css'] ) : self::get_this_css('select_style');
		$string .= ' id="escf_field' . self::$form_id_num . '_' . $key . 'm" name="' . $field['slug'] . '[m]">' . "\n";
		$selected = '';
		// minutes
        $string .= '        <option value=""></option>' . "\n";
		...
		if ( self::$form_options['time_format'] == '12' ) {
			$string .= "\n      <select ";
			$string .= ($field['input_css'] != '') ? self::convert_css( $field['input_css'] ) : self::get_this_css('select_style');
			$string .= ' id="escf_field' . self::$form_id_num . '_' . $key . 'ap" name="' . $field['slug'] . '[ap]">' . "\n";
			$selected = '';
			// am/pm
            $string .= '        <option value=""></option>' . "\n"; 
			foreach ( array( esc_html( __( 'AM', 'easy-softonic-contact-form' ) ), esc_html( __( 'PM', 'easy-softonic-contact-form' ) ) ) as $k ) {
	...
	static function get_contact_list($frm_id, $email_to) {
		...
		if (self::$global_options['enable_php_sessions'] == 'true') { // this feature only works when PHP sessions are enabled		
		  if ( ! empty($_SESSION["fsc_shortcode_email_to_$frm_id"]) &&
               preg_match( "/,/", $_SESSION["fsc_shortcode_email_to_$frm_id"]  ) ) {
				...
				if ( $key != '' && $value != '' ) {
					$contacts[] = array( 'CONTACT' => ESCF_Util::clean_input($key), 
										 'EMAIL'   => ESCF_Util::clean_input($value) );
				}
		  } else {
		...
		if ( count($contacts) == 1 ) {
			...
			if ( ! preg_match( "/,/", $contacts_test ) ) {
				if ( ESCF_Util::validate_email( $contacts_test ) ) {
					// user1@example.com
					$contacts[] = array( 'CONTACT'	 => __( 'Webmaster', 'easy-softonic-contact-form' ), 'EMAIL'		 => $contacts_test );
				}
			} else {
				$ctf_ct_arr = explode( "\n", $contacts_test );
	...
	static function get_var($form_id_num,$name) {
		...
		$value = (isset( $_GET["$form_id_num$name"])) ? ESCF_Util::clean_input($_GET["$form_id_num$name"]) : '';
		return $value;
	}
	...
	static function get_query_parms() {
		...
		foreach ( $escf_special_slugs as $fld_name ) {
		...
		foreach (self::$form_options['fields'] as $key => $field ) {
			...
			switch ( $field['type'] ) {
				...
				case 'date' :
					...
					if ( '' != $new_date & ESCF_Process::validate_date( $new_date, self::$form_id_num ) )
						self::$form_content[$fld_name] = $new_date;
					// XXX Consider adding an error or warning message if date field query parm is invalid
					if ( empty( self::$form_content[$fld_name] ) && '' != $field['default'] && ESCF_Process::validate_date( $field['default'], self::$form_id_num ) ) {
	...
	static function echo_if_error( $this_error ) {
		...
		// Settings errors begin with escf . $form_num (always shown)
	...
	static function is_captcha_enabled($frm_num) {
		...
		if ( ! self::$form_options )
			self::$form_options = ESCF_Util::get_form_options ( $frm_num, $use_defaults = true );
	...
	static function display_captcha() {
		...
		if ( !extension_loaded( 'gd' ) || !function_exists( 'gd_info' ) ) {
			$captchaRequiresError .= '<p ' . self::get_this_css('error_style') . '>' . __( 'ERROR: easy-softonic-contact-form.php plugin says GD image support not detected in PHP!', 'easy-softonic-contact-form' ) . '</p>';
			$captchaRequiresError .= '<p>' . __( 'Contact your web host and ask them why GD image support is not enabled for PHP.', 'easy-softonic-contact-form' ) . '</p>';
			$captcha_ok = false;
		}
		if ( !function_exists( 'imagepng' ) ) {
			$captchaRequiresError .= '<p ' . self::get_this_css('error_style') . '>' . __( 'ERROR: easy-softonic-contact-form.php plugin says imagepng function not detected in PHP!', 'easy-softonic-contact-form' ) . '</p>';
			$captchaRequiresError .= '<p>' . __( 'Contact your web host and ask them why imagepng function is not enabled for PHP.', 'easy-softonic-contact-form' ) . '</p>';
			$captcha_ok = false;
		}
		if ( !@strtolower( ini_get( 'safe_mode' ) ) == 'on' && !file_exists( ESCF_CAPTCHA_PATH . '/securimage.php' ) ) {
			$captchaRequiresError .= '<p ' . self::get_this_css('error_style') . '>' . __( 'ERROR: easy-softonic-contact-form.php plugin says captcha_library not found.', 'easy-softonic-contact-form' ) . '</p>';
			$captcha_ok = false;
		}
		...
		if ( $captcha_ok ) {
			...
			// the captch html
			$string = "\n<div " . self::get_this_css('title_style') . ">\n</div>\n" . '<div id="escf_captcha_image_div'.self::$form_id_num.'" ';
			...
			if ( !$enable_php_sessions ) { // no sessions
				self::init_temp_dir( ESCF_CAPTCHA_PATH . '/cache/' );

				// clean out old captcha cache files
				self::clean_temp_dir( ESCF_CAPTCHA_PATH . '/cache/' );
			...
			$string .= ">\n" . '      <img id="escf_captcha_image' . self::$form_id_num . '" ';
			$string .= self::get_this_css('captcha_image_style');
			$string .= ' src="' . $securimage_show_url . '" ' . $securimage_size . ' alt="';
			$string .= (self::$form_options['tooltip_captcha'] != '') ? esc_html( self::$form_options['tooltip_captcha'] ) : esc_html( __( 'CAPTCHA Image', 'easy-softonic-contact-form' ) );
			$string .='" title="';
			$string .= (self::$form_options['tooltip_captcha'] != '') ? esc_html( self::$form_options['tooltip_captcha'] ) : esc_html( __( 'CAPTCHA Image', 'easy-softonic-contact-form' ) );
			$string .= '" />' . "\n";

			if ( !$enable_php_sessions ) { // no sessions
				$string .= '      <input id="escf_captcha_prefix' . self::$form_id_num . '" type="hidden" name="escf_captcha_prefix' . self::$form_id_num . '" value="' . esc_attr( $prefix ) . '" />' . "\n";
			}

			$string .= '     <div id="escf_captcha_refresh' . self::$form_id_num . '">' . "\n";
			$string .= '        <a href="#" rel="nofollow" title="';
			$string .= (self::$form_options['tooltip_refresh'] != '') ? esc_html( self::$form_options['tooltip_refresh'] ) : esc_html( __( 'Refresh Image', 'easy-softonic-contact-form' ) );
			if ( !$enable_php_sessions ) { // no sessions
				$string .= '" onclick="escf_captcha_refresh(\'' . self::$form_id_num . '\',\'' . $securimage_url . '\',\'' . $securimage_show_rf_url . '\'); return false;">' . "\n";
			} else {
				$string .= '" onclick="document.getElementById(\'escf_captcha_image' . self::$form_id_num . '\').src = \'' . $securimage_show_url . '&amp;sid=\'' . ' + Math.random(); return false;">' . "\n";
			}
...
<div id="escf_captcha_field'.self::$form_id_num.'" ' . self::get_this_css('title_style') . '>
     <label '.self::get_this_css('label_style').' for="escf_captcha_code' . self::$form_id_num . '">';
			$string .= (self::$form_options['title_capt'] != '') ? self::$form_options['title_capt'] : __( 'CAPTCHA Code:', 'easy-softonic-contact-form' );
			$string .= self::$req_field_ind . '</label>
</div>
<div ' . self::get_this_css('field_div_style') . '>'
					. self::echo_if_error( 'captcha' )
					. "\n     <input " . self::get_this_css('captcha_input_style')
					. ' type="text" value="" id="escf_captcha_code' . self::$form_id_num . '" name="captcha_code" ' . self::$aria_required . ' />';
			$string .= "\n</div>";
		} else {
	...
	static function display_thank_you() {
		...
		$ctf_form_style = ESCF_Display::get_this_css('form_style');

		$ctf_thank_you = '
<!-- Easy Softonic Contact Form plugin '.esc_html(ESCF_VERSION).' - begin - EasySoftonicContactForm.com -->
<div id="FSContact' . self::$form_id_num . '" ' . $ctf_form_style.'>
';

		if (self::$form_options['border_enable'] == 'true') {
			$ctf_thank_you .= '<fieldset id="escf_form_fieldset' . self::$form_id_num . '" '. ESCF_Display::get_this_css('border_style') . '>
';
		if (self::$form_options['title_border'] != '')
		...
		$ctf_thank_you .= '
	<div id="escf_redirect'.self::$form_id_num.'" '.ESCF_Display::get_this_css('redirect_style').'>
';
		$text_message_sent = (self::$form_options['text_message_sent'] != '') ? self::$form_options['text_message_sent'] : __('Your message has been sent, thank you.', 'easy-softonic-contact-form'); // can have HTML
         $ctf_thank_you .= $text_message_sent;
		if (ESCF_Process::$redirect_enable == 'true') {
			$ctf_thank_you .= '
		<br />
		<img id="escf_redirect_image'.self::$form_id_num.'" src="'.plugins_url( 'easy-softonic-contact-form/includes/ctf-loading.gif' ).'" alt="'.esc_attr(__('Redirecting', 'easy-softonic-contact-form')).'" />'.
		'<span id="escf_redirect_word'.self::$form_id_num.'">'.__('Redirecting', 'easy-softonic-contact-form').'</span>';
		} else {
         if (self::$form_options['print_form_enable'] == 'true'){

 $ctf_thank_you .= '
<br />
<input type="button" id="escf_print_button'.self::$form_id_num.'" value="';
 $ctf_thank_you .= (self::$form_options['text_print_button'] != '') ? self::$form_options['text_print_button'] : __('View / Print your message', 'easy-softonic-contact-form');
 $ctf_thank_you .= '" onclick="escfPrintContent(\'escf_print_div\'); return false;" />
<div id="escf_print_div" style="display:none">
<h2>'.$text_message_sent.'</h2>

<p>';

 $msg_print = esc_html(ESCF_Process::$email_msg_print);
...
';
		...
		$ctf_thank_you .= '<!-- Easy Softonic Contact Form plugin '.esc_html(ESCF_VERSION).' - end - EasySoftonicContactForm.com -->
';

     //filter hook for thank_you_message
	  return apply_filters( 'es_contact_thank_you_message', $ctf_thank_you,  self::$form_id_num);
	}
	
	static function set_form_error($fld, $msg) {
		// Sets a form error for field $fld with message $msg
		// This is called from ESCF_Process class functions
		// The key is 'field' plus the field index number, or a special name such as
		// capctha, f_name, escf_select, etc.
	...
	static function check_form_errors() {
		...
		// Called from ESCF_Process::process_form()
	...
	static function ctf_notes($notes) {
		...
		return apply_filters( 'es_contact_html_before_after', $html,  self::$form_id_num);
	}
	...
	// returns the URL for the WP page the form was on
		...
		return apply_filters( 'es_contact_form_action_url', $form_action_url,  self::$form_id_num);
	...
	static function display_vcita_scheduler_button( $string ) {
	  ...
	  if (self::$form_options['vcita_scheduling_button'] == 'true'){
        $string .= '<div id="escf_button_div_vcita' . self::$form_id_num . '" '.self::get_this_css('vcita_div_button_style'). ">\n<a ".'id="escf_button_vcita' . self::$form_id_num . '" ' . self::get_this_css('vcita_button_style');

...
/**
 * Description of class-escf-util
...
class ESCF_Util {
	...
	static function setup() {
		...
		add_action('plugins_loaded', 'ESCF_Util::escf_init_languages');

        // imports old settings on plugin activate or first time upgrade from 3.xx to 4.xx
        add_action('init', 'ESCF_Util::import',1);

		// will start PHP session only if they are enabled (not enabled by default)
		add_action('init', 'ESCF_Util::escf_init_session',1);

		// process the form POST logic
		add_action('init', 'ESCF_Process::process_form',10);

		// use shortcode to print the contact form or process contact form logic
		// can use dashes or underscores: [easy-softonic-contact-form] or [es_contact_form]
		add_shortcode('es_contact_form', 'ESCF_Display::process_short_code', 1);
		add_shortcode('easy-softonic-contact-form', 'ESCF_Display::process_short_code', 1);
		...
		if ( is_admin() ) {
			...
			add_action( 'admin_menu', 'ESCF_Options::register_options_page' );

            // imports old settings on plugin activate or first time upgrade from 3.xx to 4.xx
            add_action( 'admin_init', 'ESCF_Util::import',1 );

			add_action( 'admin_init', 'ESCF_Options::initialize_options' );

			add_action( 'admin_notices', 'ESCF_Util::admin_notice' );

			add_action( 'admin_enqueue_scripts', 'ESCF_Util::enqueue_admin_scripts' );

            add_action( 'admin_footer', 'ESCF_Util::escf_admin_footer' );

			// adds "Settings" link to the plugin action page
			add_filter( 'plugin_action_links', 'ESCF_Util::escf_plugin_action_links',10,2);
		} else {
              add_action( 'wp_footer', 'ESCF_Util::escf_wp_footer' );
		}
	...
	static function import() {
		...
		// Load global options
		self::$global_options = get_option( 'es_contact_global' );
		...
		// New options table entries for individual forms will be created by ESCF_Options::get_options()
	...
	static function import_forced($force = '') {
			...
			if ($old_global_options) {
				...
				ESCF_Import::import_old_version($force);
			} else {
				...
				$temp = get_option( 'es_contact_form' );
				if ( ! empty($temp) ) {
					ESCF_Util::add_admin_notice(__( '<b>Warning</b>: Easy Softonic Contact Form cannot import settings because the previous version is too old. Installed as new.', 'easy-softonic-contact-form' ), 'error');
	...
	static function escf_init_languages() {
		if (function_exists('load_plugin_textdomain')) {
			load_plugin_textdomain('easy-softonic-contact-form', false, 'easy-softonic-contact-form/languages' );
		}
	}
	
	static function escf_init_session() {
	...
	static function enqueue_admin_scripts( $hook ) {
		...
		if( strpos ( $hook, 'easy-softonic-contact-form' ) > 0 ) {
			...
			wp_enqueue_script( 'escf_scripts_admin', plugins_url( 'easy-softonic-contact-form/includes/escf-scripts-admin.js' ), false, ESCF_BUILD );
		   	wp_enqueue_script( 'escf_scripts', plugins_url( 'easy-softonic-contact-form/includes/escf-scripts.js' ), false, ESCF_BUILD );
			// Load jquery-ui css, depending on WP version
			if ( version_compare( get_bloginfo('version'), '3.5', '<' ) )
				wp_enqueue_style( 'jquery-ui', plugins_url( 'easy-softonic-contact-form/includes/jquery-ui-1820.css' ) );
			else
				wp_enqueue_style( 'jquery-ui', plugins_url( 'easy-softonic-contact-form/includes/jquery-ui-191.css' ) );

			// Set up array of phrases used in scripts-admin.js for translation
			// NOTE: Some of the phrases below are used to match button values in ESCF_Option.
			...
			$translation_array = array(
				'save_changes' => __('Save Changes', 'easy-softonic-contact-form'),
				'send_test' => __('Send Test', 'easy-softonic-contact-form'),
				'copy_settings' => __('Copy Settings', 'easy-softonic-contact-form'),
				'backup_settings' => __('Backup Settings', 'easy-softonic-contact-form'),
				'restore_settings' => __('Restore Settings', 'easy-softonic-contact-form'),
				'confirm_change' => __('Are you sure you want to permanently make this change?', 'easy-softonic-contact-form'),
				'unsaved_changes' => __('You have unsaved changes.', 'easy-softonic-contact-form'),
				'reset_form' => __('Reset Form', 'easy-softonic-contact-form'),
                'reset_all_styles' => __('Reset Styles on all forms', 'easy-softonic-contact-form'),
				'delete_form' => __('Delete Form', 'easy-softonic-contact-form'),
                'import_old_forms' => __('Import forms from 3.xx version', 'easy-softonic-contact-form'),
				);
			wp_localize_script( 'escf_scripts_admin', 'escf_transl', $translation_array );
			wp_enqueue_style( 'escf-styles-admin', plugins_url( 'easy-softonic-contact-form/includes/escf-styles-admin.css' ), false, ESCF_BUILD );
		}
	}
    ...
    static function add_date_js() {
        ...
        //wp_enqueue_style( 'escf_date_style', plugins_url( 'easy-softonic-contact-form/date/ctf_epoch_styles.css' ), false, ESCF_BUILD );
        wp_enqueue_script( 'escf_date_js', plugins_url( 'easy-softonic-contact-form/date/ctf_epoch_classes.js' ), false, ESCF_BUILD );

        echo ESCF_Display::$add_date_js;

        $string = '  var';
		$date_var_string = '';
		foreach ( ESCF_Display::$add_date_js_array as $v ) {
			$date_var_string .= ' dp_cal' . "$v,";
		}
		$date_var_string = substr( $date_var_string, 0, -1 );
		$string .= "$date_var_string;\n";
        if (ESCF_Display::$escf_use_window_onload)
		    $string .= '  window.onload = function () {
';
		foreach ( ESCF_Display::$add_date_js_array as $v ) {
			$string .= "    dp_cal$v = new Epoch('epoch_popup$v','popup',document.getElementById('escf_field$v'));\n";
		}
        if (ESCF_Display::$escf_use_window_onload)
		   $string .= "  };\n";
...
var escf_css = "\n\
<style type='text/css'>\n\
@import url('<?php echo plugins_url( 'easy-softonic-contact-form/date/ctf_epoch_styles.css').'?ver='.ESCF_BUILD; ?>');\n\
</style>\n\
";
jQuery(document).ready(function($) {
$('head').append(escf_css);
});
//]]>
</script>
<?php

        echo "<!-- Easy Softonic Contact Form plugin - end date field js -->\n\n";

    }

	static function escf_wp_footer() {
		// Add js and css needed for the forms

		if ( isset(ESCF_Display::$add_escf_script) && ESCF_Display::$add_escf_script ) {
			...
			//wp_enqueue_style( 'escf-styles', plugins_url( 'easy-softonic-contact-form/includes/escf-styles.css' ), false, ESCF_BUILD );
			wp_enqueue_script( 'escf_scripts', plugins_url( 'easy-softonic-contact-form/includes/escf-scripts.js' ), false, ESCF_BUILD );
		}
        if ( isset(ESCF_Display::$add_placeholder_script) && ESCF_Display::$add_placeholder_script ) {
            // makes placeholder work on old browsers
            wp_enqueue_script( 'escf_placeholders', plugins_url( 'easy-softonic-contact-form/includes/escf-placeholders.min.js' ), false, ESCF_BUILD );
        }
        if ( isset(ESCF_Display::$add_date_js) && ESCF_Display::$add_date_js != '' ) {
            // add js for forms with date fields
            ESCF_Util::add_date_js();
        }
	}

    static function escf_admin_footer() {
		// add placeholder javascript in form preview page only if needed
        if ( isset(ESCF_Display::$placeholder) && ESCF_Display::$placeholder) {
             // makes placeholder work on old browsers
             wp_enqueue_script( 'escf_placeholders', plugins_url( 'easy-softonic-contact-form/includes/escf-placeholders.min.js' ), false, ESCF_BUILD );
        }
        if ( isset(ESCF_Display::$add_date_js) && ESCF_Display::$add_date_js != '' ) {
...
';
	...
	static function escf_plugin_action_links( $links, $file ) {
		//Static so we don't call plugin_basename on every plugin row.
		static $this_plugin;
		if ( ! $this_plugin )
			$this_plugin = plugin_basename( ESCF_FILE );

		if ( $file == $this_plugin ) {
			$settings_link = '<a href="plugins.php?page=easy-softonic-contact-form/easy-softonic-contact-form.php">' . __( 'Settings', 'easy-softonic-contact-form' ) . '</a>';
			array_unshift( $links, $settings_link ); // before other links
		}
		return $links;
	} // end function escf_plugin_action_links
		...
		// Load global options
			...
			if ( ! self::$global_options ) {
				...
				if ( !isset( self::$global_defaults ) )
					self::set_defaults( );
				update_option( 'es_contact_global', self::$global_defaults );
				self::$global_options = get_option( 'es_contact_global' );
			}
	...
	static function get_form_options( $form_num, $use_defaults ) {
		...
		if ( is_numeric($form_num) && $form_num > 0 ) {
			// Load form options
			$form_option_name = 'es_contact_form' . $form_num;
			...
			if ( ! $form_options && $use_defaults ) {
				...
				if ( "" == self::$form_defaults['form_name'] )
					self::$form_defaults['form_name'] = __('Form', 'easy-softonic-contact-form') .' '. $form_num;
	...
	static function update_options_version() {
		...
		if ( version_compare( ESCF_VERSION, self::$global_options['escf_version'], '>' ) ) {
			// Update the global options
			// Unset any removed global options here:
			//

			if ( version_compare( self::$global_options['escf_version'], '1.0', '<' ) ) {
			...
			if ( !isset( self::$global_defaults ) )
				self::set_defaults();
			self::$global_options = array_merge( self::$global_defaults, self::$global_options );
			self::$global_options['escf_version'] = ESCF_VERSION;
			update_option( 'es_contact_global', self::$global_options );

			// Update the form options
			foreach ( self::$global_options['form_list'] as $key => $form ) {
				self::$form_options = get_option( 'es_contact_form' . $key );
				if ( self::$form_options ) {
					...
					update_option( 'es_contact_form' . $key, self::$form_options );
				}
	...
	static function set_defaults() {
		...
		self::$form_defaults = array(
			 'form_name' => __('New Form', 'easy-softonic-contact-form'),
			 'welcome' => __('<p>Welcome to the Contact Form.</p>', 'easy-softonic-contact-form'),
             'after_form_note' => '',
			 'email_to' => __('Webmaster', 'easy-softonic-contact-form').','.get_option('admin_email'),
			 'php_mailer_enable' => 'wordpress',
			 'email_from' => '',
			 'email_from_enforced' => 'false',
			 'email_reply_to' => '',
			 'email_bcc' => '',
			 'email_subject' => get_option('blogname') . ' ' .__('Contact:', 'easy-softonic-contact-form'),
			 'email_subject_list' => '',
			 'name_format' => 'name',
             ...
             'submit_attributes' => '',
             'success_page_html' => '',
			 'title_border' => __( 'Contact Form', 'easy-softonic-contact-form' ),
			 'title_dept' => '',
			 'title_select' => '',
			 'title_name' => '',
			 'title_fname' => '',
		...
		$name = array(
			...
			'req'			 => 'true',
			'label'			 => __('Name:', 'easy-softonic-contact-form'),
			'slug'			 =>	'full_name',
			'type'			 => 'text'
		);
		 $email = array(
			...
			'req'			 => 'true',
			'label'			 => __('Email:', 'easy-softonic-contact-form'),
			'slug'			 =>	'email',
			'type'			 => 'text'
		);
		...
		$subject = array(
			...
			'req'			 => 'true',
			'label'			 => __('Subject:', 'easy-softonic-contact-form'),
			'slug'			 =>	'subject',
			'type'			 => 'text'
		);
		$message = array(
			...
			'req'			 => 'true',
			'label'			 => __('Message:', 'easy-softonic-contact-form'),
			'slug'			 =>	'message',
			'type'			 => 'textarea'
		);
		...
		// Called by set_defaults() and ESCF_Options::validate()
	...
	static function get_field_defaults() {
		...
		self::$field_defaults = array(
			...
			'label'			 => __('New Field:', 'easy-softonic-contact-form'),
	...
	static function update_lang(&$form_options) {
		//  global ESCF_Options::$form_options, ESCF_Options::$form_optionsion_defaults;
		...
		if ( $form_options['welcome'] == '<p>Welcome to the Contact Form.</p>' ) {
			$form_options['welcome'] = __( '<p>Welcome to the Contact Form.</p>', 'easy-softonic-contact-form' );
		}

		if ( $form_options['email_to'] == 'Webmaster,' . get_option( 'admin_email' ) ) {
			$form_options['email_to'] = __( 'Webmaster', 'easy-softonic-contact-form' ) . ',' . get_option( 'admin_email' );
		}

		if ( $form_options['email_subject'] == get_option( 'blogname' ) . ' ' . 'Contact:' ) {
			$form_options['email_subject'] = get_option( 'blogname' ) . ' ' . __( 'Contact:', 'easy-softonic-contact-form' );
		}
	}	// end function es_contact_update_lang
	...
	static function validate_email($email) {
       ...
       if (!empty(ESCF_Process::$form_options['email_check_easy']) && ESCF_Process::$form_options['email_check_easy'] == 'true') {
	   ...
	   if (!empty(ESCF_Process::$form_options['email_check_dns']) && ESCF_Process::$form_options['email_check_dns'] == 'true') {
	...
	static function get_captcha_url_cf() {
	  ...
	  $captcha_url_cf  = plugins_url( 'captcha' , ESCF_FILE );

	  if ($site_uri['host'] == $home_uri['host']) {
		  // use $captcha_url_cf above
	  } else {
		  $captcha_url_cf  = get_option( 'home' ) . '/'.PLUGINDIR.'/easy-softonic-contact-form/captcha';
	  }
	...
	// Consider moving this function to ESCF_Process

		if ( ESCF_Process::$form_options['name_case_enable'] !== 'true' ) {
	...
	static function validate_url($url) {
		...
		$regex .= "(\:[0-9]{2,5})?"; // Port
		$regex .= "(\/#\!)?"; // Path hash bang  (twitter) (Umair Saleem added)
